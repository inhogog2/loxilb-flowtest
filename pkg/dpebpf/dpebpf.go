...
type DpCtInfo struct {
	...
	// Flow Exporter Additional Data
	STs          time.Time `json:"startTime"`
	ETs          time.Time `json:"endTime"`
	DeltaPackets uint64    `json:"dpackets"`
	DeltaBytes   uint64    `json:"dbytes"`
	// Delete flag
	DeleteFlag    bool
	FlowEndReason uint8
	UpdateFlag    bool
...
func (ct Maps) GetMaps() {
	for k, v := range ct.CtMap {
		fmt.Printf("k: %v\n", k)
		fmt.Printf("v.STs: %v\n", v.STs)
		fmt.Printf("v.ETs: %v\n", v.ETs)
	}
...
func goMapNotiHandler(m *mapNoti) {
	...
	ctMap.CtMap[goCtEnt.Key()] = goCtEnt
	//tk.LogIt(tk.LogDebug, "[CT] %s -Is added \n", goCtEnt.String())
}
...
func dpCTMapChkUpdates() {
	...
	for _, cti := range ctMap.CtMap {
		...
		if cti.CState != "est" {
			...
			goCtEnt.LTs = tc
			if goCtEnt.CState != cti.CState || goCtEnt.CAct != cti.CState {
				...
				// Make get Delta Stats
				goCtEnt.DeltaPackets = goCtEnt.Packets - prePackets
				goCtEnt.DeltaBytes = goCtEnt.Bytes - preBytes
				goCtEnt.UpdateFlag = true
				delete(ctMap.CtMap, cti.Key())
				ctMap.CtMap[goCtEnt.Key()] = goCtEnt
			...
			if time.Duration(tc.Sub(cti.LTs).Seconds()) >= time.Duration(5*60) {
				...
				if C.bpf_map_lookup_elem(C.int(fd), unsafe.Pointer(&cti.PKey[0]), unsafe.Pointer(&tact)) != 0 {
					...
					// 삭제전 확인.
					delete(ctMap.CtMap, cti.Key())
					tk.LogIt(tk.LogDebug, "[CT] DELETE3 %s\n", cti.Key())
					continue
				}
...
func (ct *DpCtInfo) convDPCt2GoObjFixup(ctKey *C.struct_dp_ct_key, ctDat *C.struct_dp_ct_dat, fixup bool) *DpCtInfo {
	...
	// FlowExporter Additional Data check
	ct.STs = time.Now()
	ct.ETs = time.Now()
	ct.DeltaPackets = uint64(ctDat.pb.packets)
	ct.DeltaBytes = uint64(ctDat.pb.bytes)
	ct.DeleteFlag = false
	ct.FlowEndReason = 0
	ct.UpdateFlag = false
	return ct
